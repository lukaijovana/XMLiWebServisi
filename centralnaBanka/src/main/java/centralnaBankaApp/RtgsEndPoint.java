	...
	public Odgovor103 getNalogZaPrenosRequest(@RequestPayload Zahtev103 request) {
		...
		if(zaZaduziti != null && zaOdobriti != null) {
			...
			if(obracunskiDuznika >= 0) {
				...
				MT900 odg = new MT900();
				odg.setIDPoruke("9");
				odg.setSWIFTDuznika(nalog.getDuznik().getSWIFT());
				odg.setDatum(nalog.getDatumValute());
				odg.setIDPorukeNaloga(nalog.getIDPoruke());
				odg.setIznos(nalog.getIznos());
				odg.setRacunBnkDuznika(nalog.getDuznik().getObracunskiRacun());
				...
				mt910.setDatum(nalog.getDatumValute());
